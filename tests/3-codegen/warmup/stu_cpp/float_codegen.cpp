...
void translate_main(CodeGen *codegen) {
    ...
    offset_map["*%op0"] = -(16+8+4); // TODO: 请填空
    ...
    // %op1 对应的内存空间中
    offset_map["%op1"] = -(16+8+4+4); // TODO: 请填空
    ...
    offset_map["%op2"] = -(16+8+4+4+4); // TODO: 请填空
    codegen->append_inst("ld.w", {"$t0", "$fp", std::to_string(offset_map["%op1"])});
    codegen->append_inst("movgr2fr.w $f0, $t0");
    codegen->append_inst("ftintrz.w.s $f1, $f0");
    codegen->append_inst("movfr2gr.s $t1, $f1");
